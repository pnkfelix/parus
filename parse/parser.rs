...
use syntax::ast::{Sigil, BorrowedSigil, ManagedSigil, OwnedSigil};
use syntax::ast::{CallSugar, NoSugar, DoSugar};
use syntax::ast::{TyBareFn, TyClosure};
use syntax::ast::{RegionTyParamBound, TraitTyParamBound};
use syntax::ast::{provided, public, purity};
use syntax::ast::{_mod, BiAdd, arg, Arm, Attribute, BindByRef, BindByValue};
use syntax::ast::{BiBitAnd, BiBitOr, BiBitXor, Block};
use syntax::ast::{BlockCheckMode, UnBox};
use syntax::ast::{Crate, CrateConfig, Decl, DeclItem};
use syntax::ast::{DeclLocal, DefaultBlock, UnDeref, BiDiv, EMPTY_CTXT, enum_def, explicit_self};
use syntax::ast::{Expr, Expr_, ExprAddrOf, ExprMatch, ExprAgain};
use syntax::ast::{ExprAssign, ExprAssignOp, ExprBinary, ExprBlock};
use syntax::ast::{ExprBreak, ExprCall, ExprCast, ExprDoBody};
use syntax::ast::{ExprField, ExprFnBlock, ExprIf, ExprIndex};
use syntax::ast::{ExprLit, ExprLogLevel, ExprLoop, ExprMac};
use syntax::ast::{ExprMethodCall, ExprParen, ExprPath, ExprProc, ExprRepeat};
use syntax::ast::{ExprRet, ExprSelf, ExprStruct, ExprTup, ExprUnary};
use syntax::ast::{ExprVec, ExprVstore, ExprVstoreMutBox};
use syntax::ast::{ExprVstoreSlice, ExprVstoreBox};
use syntax::ast::{ExprVstoreMutSlice, ExprWhile, ExprForLoop, extern_fn, Field, fn_decl};
use syntax::ast::{ExprVstoreUniq, Onceness, Once, Many};
use syntax::ast::{foreign_item, foreign_item_static, foreign_item_fn, foreign_mod};
use syntax::ast::{Ident, impure_fn, inherited, item, item_, item_static};
use syntax::ast::{item_enum, item_fn, item_foreign_mod, item_impl};
use syntax::ast::{item_mac, item_mod, item_struct, item_trait, item_ty, lit, lit_};
use syntax::ast::{lit_bool, lit_float, lit_float_unsuffixed, lit_int, lit_char};
use syntax::ast::{lit_int_unsuffixed, lit_nil, lit_str, lit_uint, Local};
use syntax::ast::{MutImmutable, MutMutable, mac_, mac_invoc_tt, matcher, match_nonterminal};
use syntax::ast::{match_seq, match_tok, method, mt, BiMul, Mutability};
use syntax::ast::{named_field, UnNeg, noreturn, UnNot, P, Pat, PatBox, PatEnum};
use syntax::ast::{PatIdent, PatLit, PatRange, PatRegion, PatStruct};
use syntax::ast::{PatTup, PatUniq, PatWild, PatWildMulti, private};
use syntax::ast::{BiRem, required};
use syntax::ast::{ret_style, return_val, BiShl, BiShr, Stmt, StmtDecl};
use syntax::ast::{StmtExpr, StmtSemi, StmtMac, struct_def, struct_field};
use syntax::ast::{struct_variant_kind, BiSub};
use syntax::ast::StrStyle;
use syntax::ast::{sty_box, sty_region, sty_static, sty_uniq, sty_value};
use syntax::ast::{token_tree, trait_method, trait_ref, tt_delim, tt_seq, tt_tok};
use syntax::ast::{tt_nonterminal, tuple_variant_kind, Ty, ty_, ty_bot, ty_box};
use syntax::ast::{TypeField, ty_fixed_length_vec, ty_closure, ty_bare_fn, ty_typeof};
use syntax::ast::{ty_infer, TypeMethod};
use syntax::ast::{ty_nil, TyParam, TyParamBound, ty_path, ty_ptr, ty_rptr};
use syntax::ast::{ty_tup, ty_u32, ty_uniq, ty_vec, UnUniq};
use syntax::ast::{unnamed_field, UnsafeBlock, unsafe_fn, view_item};
use syntax::ast::{view_item_, view_item_extern_mod, view_item_use};
use syntax::ast::{view_path, view_path_glob, view_path_list, view_path_simple};
use syntax::ast::visibility;
use syntax::ast;
use syntax::ast_util::{as_prec, operator_prec};
use syntax::ast_util;
use syntax::attr;
use syntax::codemap::{Span, BytePos, Spanned, spanned, mk_sp};
use syntax::codemap;
use r_parse = syntax::parse;
use syntax::parse::obsolete;
use syntax::parse::attr::parser_attr;
use syntax::parse::classify;
use syntax::parse::common::{SeqSep, seq_sep_none};
use syntax::parse::common::{seq_sep_trailing_disallowed, seq_sep_trailing_allowed};
use syntax::parse::lexer::reader;
use syntax::parse::lexer::TokenAndSpan;
use syntax::parse::obsolete::ParserObsoleteMethods;
use syntax::parse::token::{can_begin_expr, get_ident_interner, ident_to_str, is_ident};
use syntax::parse::token::{is_ident_or_path};
use syntax::parse::token::{is_plain_ident, INTERPOLATED, keywords, special_idents};
use syntax::parse::token::{token_to_binop};
...
pub fn Parser(sess: @mut r_parse::ParseSess,
              cfg: ast::CrateConfig,
              rdr: @mut reader)
           -> Parser {
...
pub struct Parser {
    ...
    obsolete_set: @mut HashSet<obsolete::ObsoleteSyntax>,
...
impl ParserObsoleteMethods for Parser {
    ...
    fn obsolete_expr(&self, sp: Span, kind: obsolete::ObsoleteSyntax) -> @Expr {
        self.obsolete(sp, kind);
        self.mk_expr(sp.lo, sp.hi, ExprLit(@codemap::respan(sp, lit_nil)))
    }

    fn report(&self,
              sp: Span,
              kind: obsolete::ObsoleteSyntax,
              kind_str: &str,
              desc: &str) {
    ...
    fn token_is_obsolete_ident(&self, ident: &str, token: &token::Token)
                                   -> bool {
        match *token {
            token::IDENT(sid, _) => {
...
impl Parser {
    ...
    pub fn parse_box_or_uniq_pointee(&self,
        ...
        match *self.token {
            token::LIFETIME(..) => {
                ...
                self.obsolete(*self.last_span, obsolete::ObsoleteBoxedClosure);
    ...
    pub fn parse_borrowed_pointee(&self) -> ty_ {
        ...
        if self.token_is_old_style_closure_keyword() {
            self.obsolete(*self.last_span, obsolete::ObsoleteClosureType);
    ...
    pub fn parse_mutability(&self) -> Mutability {
        if self.eat_keyword(keywords::Mut) {
            ...
            self.obsolete(*self.last_span, obsolete::ObsoleteConstPointer);
            MutImmutable
        } else {
            MutImmutable
        }
    ...
    pub fn parse_assign_expr(&self) -> @Expr {
        ...
        match *self.token {
          ...
          token::DARROW => {
            self.obsolete(*self.span, obsolete::ObsoleteSwap);
            self.bump();
    ...
    pub fn parse_loop_expr(&self, opt_ident: Option<ast::Ident>) -> @Expr {
        ...
        if is_loop_header {
            ...
            self.obsolete(*self.last_span, obsolete::ObsoleteLoopAsContinue);
            let lo = self.span.lo;
            let ex = if self.token_is_lifetime(&*self.token) {
                let lifetime = self.get_lifetime(&*self.token);
                self.bump();
                ...
                if *self.token == token::COMMA || *self.token == token::RBRACKET {
                    ...
                    match subpat {
                        @ast::Pat { id, node: PatWild, span } => {
                            self.obsolete(*self.span, obsolete::ObsoleteVecDotDotWildcard);
                            slice = Some(@ast::Pat {
                                id: id,
                                node: PatWildMulti,
                                span: span
    ...
    fn parse_pat_fields(&self) -> (~[ast::FieldPat], bool) {
        ...
        while *self.token != token::RBRACE {
            ...
            if *self.token == token::UNDERSCORE {
                self.obsolete(*self.span, obsolete::ObsoleteStructWildcard);
            }
            if etc {
                self.bump();
                if *self.token != token::RBRACE {
                        ...
                        match *self.token {
                          token::LPAREN => {
                            ...
                            if is_star | is_dotdot {
                                ...
                                self.bump();
                                if is_star {
                                    self.obsolete(*self.span, obsolete::ObsoleteEnumWildcard);
                                }
                                self.bump();
                                self.expect(&token::RPAREN);
    ...
    fn parse_let(&self) -> @Decl {
        ...
        while self.eat(&token::COMMA) {
            ...
            self.obsolete(*self.span, obsolete::ObsoleteMultipleLocalDecl);
        }
    ...
    pub fn parse_block(&self) -> P<Block> {
        ...
        let lo = self.span.lo;
        if self.eat_keyword(keywords::Unsafe) {
            self.obsolete(*self.span, obsolete::ObsoleteUnsafeBlock);
        }
        self.expect(&token::LBRACE);
    ...
    fn parse_item_impl(&self) -> item_info {
        ...
        let inner_attrs = if self.eat(&token::SEMI) {
            self.obsolete(*self.last_span, obsolete::ObsoleteEmptyImpl);
            None
        } else {
            self.expect(&token::LBRACE);
    ...
    fn eval_src_mod_from_path(&self,
        ...
        let p0 =
            p_parse::new_sub_parser_from_file(self.sess,
                                     self.cfg.clone(),
                                     &path,
                                     id_sp);
    ...
    fn parse_item_foreign_fn(&self, vis: ast::visibility,
        ...
        if purity != impure_fn {
            self.obsolete(*self.last_span, obsolete::ObsoleteUnsafeExternFn);
        }
    ...
    fn parse_item_foreign_mod(&self,
        ...
        if items_allowed && self.eat(&token::LBRACE) {
            ...
            if named {
                self.obsolete(*self.last_span, obsolete::ObsoleteNamedExternModule);
            }
